...
class HomeScreen(ctk.CTk):
    ...
    def __init__(self):
        ...
        if os.path.exists("careervue.log"):
            try:
                ...
                print(f"Log file 'careervue.log' deleted successfully.")
        ...
        self.setup_header_frame()  # Header frame for kogo, add button, refresh button, etc.
        self.setup_main_frame()  # Main frame that contains job frame
        self.setup_jobs_frame()  # Scrollable frame for job entries
        ...
        # Refresh job list
        logging.info("Refreshed job list.")
        self.refresh_jobs()
        ...
        try:
            with open("last_checked.json", "r") as f:
                ...
                self.last_sync_label.configure(
                    text=f"Last sync: {last_checked.strftime('%Y-%m-%d %H:%M:%S')}"
                )
    ...
    def setup_jobs_frame(self):
        ...
        self.jobs_frame.grid_columnconfigure(
            (5, 6), weight=0
        )  # Notes and Delete columns
        ...
        for i, header in enumerate(headers):
            label = ctk.CTkLabel(
                self.jobs_frame, text=header, font=ctk.CTkFont(size=16, weight="bold")
            )
    ...
    def refresh_emails_and_jobs(self):
        ...
        if self.email_watcher:
            try:
                ...
                CTkMessagebox(
                    title="Success",
                    message="Emails checked and jobs refreshed!",
                    icon="info",
                )
            except Exception as e:
                ...
                CTkMessagebox(
                    title="Error",
                    message="Failed to refresh emails. Please try again.",
                    icon="cancel",
                )
        else:
            CTkMessagebox(
                title="Error",
                message="Email watcher not configured. Please set up email configuration first.",
                icon="cancel",
            )
    ...
    def load_config(self):
        try:
            ...
            print("Warning: The file 'email_config.json' was not found.")
            config = {}  # Assign a default empty dictionary or other default values
        except json.JSONDecodeError:
            print(
                "Error: Failed to decode JSON. The file may be corrupted or improperly formatted."
            )
            config = {}  # Assign a default empty dictionary or other default values
        except IOError as e:
    ...
    def start_email_watcher(self):
        ...
        if not all(key in self.config for key in required_keys):
            CTkMessagebox(
                title="Error",
                message="Email configuration incomplete. Please check your credentials and try again",
                icon="cancel",
            )
            return
        ...
        if self.email_watcher.connect():
            self.email_watcher_thread = threading.Thread(
                target=self.run_email_watcher, daemon=True
            )
    ...
    def run_email_watcher(self):
        ...
        while not getattr(self.email_watcher, "stop_flag", False):
            try:
                ...
                self.after(
                    0, self.refresh_jobs
                )  # Schedule job refresh on the main thread
                self.after(
                    0, self.update_sync_time
                )  # Update sync time on the main thread
                ...
                self.after(
                    0, self.update_status_indicator
    ...
    def stop_email_watcher(self):
        ...
        if (
            self.email_watcher
            and self.email_watcher_thread
            and self.email_watcher_thread.is_alive()
        ):
    ...
    def delete_job(self, job_id):
        ...
        if confirm.get() == "Yes":
            ...
            if result:
                ...
                logging.warning(
                    f"Attempted to delete non-existent job with ID {job_id}"
                )
    ...
    def update_job(self, job_id, field, value):
        ...
        try:
            current_date = datetime.now().strftime("%Y-%m-%d")
            cursor.execute(
                f"UPDATE jobs SET {field} = ?, last_updated = ? WHERE id = ?",
    ...
    def add_job_row(
        self, job_id, company, position, status, app_date, last_updated, notes, updated
    ):
        ...
        update_indicator.bind(
            "<Button-1>", lambda e, j=job_id: self.clear_update_indicator(j)
        )
        ...
        company_entry.bind(
            "<FocusOut>",
            lambda e, j=job_id, w=company_entry: self.validate_and_update(
                j, "company", w.get(), w
            ),
        )
        ...
        position_entry.bind(
            "<FocusOut>",
            lambda e, j=job_id, w=position_entry: self.validate_and_update(
                j, "position", w.get(), w
            ),
        )
        ...
        status_dropdown.configure(
            command=lambda v, j=job_id: self.update_job(j, "status", v)
        )
        ...
        app_date_entry.bind(
            "<FocusOut>",
            lambda e, j=job_id, w=app_date_entry: self.validate_and_update(
                j, "application_date", w.get(), w
            ),
        )
        ...
        if job_id in self.job_rows:
            if field == "last_updated":
                ...
                if field in self.job_rows[job_id]:
                    ...
                    print(
                        f"Warning: Field '{field}' not found in job_rows for job_id {job_id}"
                    )
            elif field == "status":
